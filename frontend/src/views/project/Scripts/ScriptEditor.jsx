...
const initialValue = [
  ...
  },
  {
    type: "action",
    children: [
      { text: "A man walks into the room, looking around suspiciously." },
    ],
  },
  {
    type: "character",
...
const ScriptEditor = () => {
  ...
  const deleteScript = async () => {
    ...
    try {
      ...
      navigate(`/project/${projectid}/scripts/`, {
        state: { message: "The script has been deleted." },
      });
    } catch (error) {
      console.error(
    ...
    editor.children.forEach((node) => {
      switch (node.type) {
        ...
        case "character":
          doc.setFont("Helvetica", "bold");
          doc.setFontSize(12);
          doc.text(node.children[0].text.toUpperCase(), 105, y, {
            align: "center",
          });
          y += 8;
          break;
  ...
  const Toolbar = () => (
    <div className="script-editor-toolbar">
      ...
      <button
        onClick={() =>
          editor.insertNode({ type: "sceneHeading", children: [{ text: "" }] })
        }
      >
        Scene Heading
      </button>
      ...
      <button
        onClick={() =>
          editor.insertNode({ type: "character", children: [{ text: "" }] })
        }
      >
        Character
      </button>
